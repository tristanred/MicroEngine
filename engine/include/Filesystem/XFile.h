...
public:
    ...
    // not exist or is invalid. Size can be 0 if we open a new file for writting.
    size_t Size;
    
    // Open the file but does not read anything. This will check file permissions,
    // ensure that the file exists and is a valid target.
...
private:
    ...
    // File path that is currently loaded. Set to NULL if the file isn't correct.
    // If this path is set, it means that the target file exists and everything
    // checks out. If loadedPath is NULL, the file is not opened or invalid.
    char* loadedPath;
